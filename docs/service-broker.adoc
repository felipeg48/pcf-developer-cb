...
. Download the following https://github.com/eitansuez/cloudfoundry-mongodb-service-broker/releases/download/v0.2/cloudfoundry-mongodb-service-broker-0.2.zip[zip file^], which contains the source code and precompiled jar file, ready for you to deploy (no building necessary).  Copy the file to folder: `~/pivotal-cloud-foundry-developer-workshop/`.
