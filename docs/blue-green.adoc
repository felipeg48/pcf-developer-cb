...
[source.terminal]
----
cf blue-green-deploy articulate -f manifest.yaml
----

Observe what this command does:

. it deploys articulate using a different application name:  `articulate-new`.  This is similar to what we did by hand when we chose to use the '-v2' suffix for the application name.  Here the plugin uses the '-new' suffix.

. the application hostname also mirrors this naming convention: articulate-new
...
And we can take this a step further, by passing a smoke-test script to the `blue-green-deploy` command to determine if the newly-deployed application passes certain smoke tests as a condition to switching over to it.
