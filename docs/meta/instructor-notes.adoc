...
Here is a link:../artifacts/create-pes-spaces.sh[shell script^] that automates the creation of a space for a student.  And here's the corresponding link:../artifacts/delete-pes-spaces.sh[cleanup script^] for cleaning up the space (and removing the user from the org) after class is over.
